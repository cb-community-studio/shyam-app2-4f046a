...
import helloFakerFactory from "./helloFakerFactory";
...
const HelloFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = helloFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hello").create(elem));
...
export default connect(null, mapDispatch)(HelloFakerDialogComponent);
